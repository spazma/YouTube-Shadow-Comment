...
(() => {
  ...
  function createShadowUI(){
    ...
    try {
      ...
      const h4 = document.createElement('h4'); h4.textContent="YSC: Lista handle'ów)"; panel.appendChild(h4);
      const info = document.createElement('div'); info.style.color='#aaa'; info.style.fontSize='12px'; info.style.marginBottom='8px';
      info.textContent = 'Author: Robert Wesner & Spazma — Shadowban detect included';
      panel.appendChild(info);
      const ta = document.createElement('textarea'); ta.id = 'ysc-targets-ta'; ta.placeholder = '@handle1\n@handle2'; panel.appendChild(ta);
      const muted = document.createElement('div'); muted.style.color='#bbb'; muted.style.fontSize='12px'; muted.style.marginTop='6px'; muted.textContent='Jedno @handle w linii'; panel.appendChild(muted);
      ...
      const btnRestore = document.createElement('button'); btnRestore.className='ysc-btn secondary'; btnRestore.textContent='Przywróć domyślne';
      const btnClear = document.createElement('button'); btnClear.className='ysc-btn secondary'; btnClear.textContent='Wyczyść cache';
      const btnCancel = document.createElement('button'); btnCancel.className='ysc-btn secondary'; btnCancel.textContent='Anuluj';
      const btnSave = document.createElement('button'); btnSave.className='ysc-btn'; btnSave.textContent='Zapisz';
